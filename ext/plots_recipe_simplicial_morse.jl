# Recipe and convenience function for SimplicialMorsePlot
...
@recipe function f(data::SimplicialMorsePlot)
    ec = data.complex
    ...
    # --- Morse sets (one or two series per set: triangles + edges + vertices) ---
    for m in eachindex(ms_int)
        ms  = ms_int[m]
        col = cols[m]
