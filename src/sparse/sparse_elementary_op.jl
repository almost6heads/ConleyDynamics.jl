...
function sparse_add_column!(matrix::SparseMatrix, ci1::Int, ci2::Int, cn, cd)
    ...
    @boundscheck begin
        if !(1 <= ci1 <= matrix.ncol) || !(1 <= ci2 <= matrix.ncol)
            throw(BoundsError(matrix, (:, (ci1, ci2))))
        end
    end
    ...
    @inbounds for k in 1:length(nonzero_rows)
...
function sparse_add_row!(matrix::SparseMatrix, ri1::Int, ri2::Int, cn, cd)
    ...
    @boundscheck begin
        if !(1 <= ri1 <= matrix.nrow) || !(1 <= ri2 <= matrix.nrow)
            throw(BoundsError(matrix, ((ri1, ri2), :)))
        end
    end
    ...
    @inbounds for kc in nonzero_columns
